...
Report 50031 DynChartDeliverySalesservice
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/DynChartDeliverySalesService.rdl';
    ...
    dataset
    {
        dataitem("Sales Shipment Line"; "Sales Shipment Line")
        {
            ...
            begin
                if DataSource <> Datasource::Sales then begin
                    CurrReport.Skip();
                end;
                ...
                doLine("Sales Shipment Line"."Promised Delivery Date", "Sales Shipment Line"."Posting Date");
            end;

            trigger OnPreDataItem()
            begin
                if DataSource = Datasource::Sales then begin
                    Chart1Caption := StrSubstNo(TextChart1Caption, "Sales Shipment Line".TableCaption());
                end;
            end;
        }
        dataitem("Service Shipment Header"; "Service Shipment Header")
        {
            RequestFilterFields = "Expected Finishing Date";
            ...
            trigger OnAfterGetRecord()
            begin
                if DataSource <> Datasource::Service then begin
                    CurrReport.Skip();
                end;

                doLine("Service Shipment Header"."Expected Finishing Date", "Service Shipment Header"."Finishing Date");
            end;

            trigger OnPreDataItem()
            begin
                if DataSource = Datasource::Service then begin
                    Chart1Caption := StrSubstNo(TextChart1Caption, "Service Shipment Header".TableCaption());
                end;
            end;
        }
        dataitem(IntegerChartData; "Integer")
        {
            ...
            column(Chart1Caption; Chart1Caption)
            {
            }
    ...
    var
        ...
        DataSource: Option Sales,Service;
        ParmInterval1: Integer;
        ParmInterval2: Integer;
        TextChart1Caption: label 'Delivery - Interval of Days (%1)';
    ...
    procedure doLine(_Date1: Date; _Date2: Date)
    var
        TempPeriod: Text[20];
        Found: Boolean;
    begin
        if (_Date1 = 0D) or (_Date2 = 0D) then begin
            CurrReport.Skip();
        end;

        TempPeriod := Format(_Date1, 0, '<Year>-<Month,2>');

        if DynChartDataDelivery.Get(TempPeriod) then begin
            Found := true;
        end else begin
            DynChartDataDelivery.PeriodKey := TempPeriod;
            DynChartDataDelivery.PeriodCaption := CopyStr(Format(_Date1, 0, '<Month Text>'), 1, 3) + ' ' + Format(_Date1, 0, '<Year,2>');
        ...
        if (_Date2 - _Date1) > 0 then begin
            if (_Date2 - _Date1) <= ParmInterval1 then begin
                DynChartDataDelivery.Interval1Qty += 1;
            end else
                if (_Date2 - _Date1) <= ParmInterval2 then begin
