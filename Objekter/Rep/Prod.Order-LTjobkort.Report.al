...
Report 50010 "Prod. Order - LT jobkort"
{
    // 191213 VD/age   Lykketronic "jobkort"
    // 060814 VD/MBP
    // 
    // //Dynalogic, srn 06112014
    // tilføjet ext.varenummer + beskrivelse
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Prod. Order - LT jobkort.rdl';
    ...
    dataset
    {
        dataitem("Production Order"; "Production Order")
        {
            ...
            column(ProdOrderEndDate; "Production Order"."Ending Date")
            {
            }
            column(NormTime; NormTime)
            {
            }
            column(NormTimeUOM; NormTimeUOM)
            {
            }
            dataitem("Prod. Order Component"; "Prod. Order Component")
            {
                ...
                begin
                    ...
                    //pos := Lowercase("Prod. Order Component".Position + "Prod. Order Component"."Position 2");  //9A Jens Hinnerup
                    pos := Lowercase("Prod. Order Component".pos30);
                    // :191213
            ...
            begin
                ...
                serialNos := '';
                noSeriesLine.SETRANGE("Series Code", item."Serial Nos.");
                IF noSeriesLine.FINDFIRST THEN BEGIN
                    serialNos := INCSTR(noSeriesLine."Last No. Used") + ' ...';
                    lastSerialNo := noSeriesLine."Last No. Used";
                    WHILE c < "Production Order".Quantity DO BEGIN
                        lastSerialNo := INCSTR(lastSerialNo);
                        c += 1;
                    END;
                    serialNos := INCSTR(noSeriesLine."Last No. Used") + ' - ' + lastSerialNo;
                END;
                // MMU
                ...
                //Dynalogic 121021 JNR >>
                Clear(NormTime);
                Clear(NormTimeUOM);
                ProdOrderLineFilter.SetRange(Status, "Production Order".Status);
                ProdOrderLineFilter.SetRange("Prod. Order No.", "Production Order"."No.");
                if ProdOrderLineFilter.FindSet then begin
                    ProdOrderRoutingLine.Reset;
                    ProdOrderRoutingLine.SetRange(Status, ProdOrderLineFilter.Status);
                    ProdOrderRoutingLine.SetRange("Prod. Order No.", ProdOrderLineFilter."Prod. Order No.");
                    ProdOrderRoutingLine.SetRange("Routing No.", ProdOrderLineFilter."Item No.");
                    if ProdOrderRoutingLine.FindSet then
                        repeat
                            NormTime := NormTime + (ProdOrderRoutingLine."Run Time" * ProdOrderLineFilter.Quantity);
                            NormTimeUOM := ProdOrderRoutingLine."Run Time Unit of Meas. Code";
