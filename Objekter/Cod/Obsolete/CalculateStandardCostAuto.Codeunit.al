...
Codeunit 50022 CalculateStandardCostAuto
{
    // //Dynalogic - JHH - Automaticering af codeunit , skal opsættes til afvikling fra Jobkø
    trigger OnRun()
    begin
    end;
    ...
    procedure CalcRtngCostPerUnit(Type: Option "Work Center","Machine Center"," "; No: Code[20]; var DirUnitCost: Decimal; var IndirCostPct: Decimal; var OvhdRate: Decimal; var UnitCost: Decimal; var UnitCostCalculation: Option Time,Unit)
    var
    ...
    begin
        ...
        //NAL_JHHCostCalcMgt.RoutingCostPerUnit2(Type, DirUnitCost, IndirCostPct, OvhdRate, UnitCost, UnitCostCalculation, WorkCenter, MachineCenter);
    end;
    ...
    procedure CalculateAssemblyCostStd(ItemNo: Code[20]; QtyBase: Decimal; var StdCost: array[5] of Decimal)
    var
        Item: Record Item;
    ...
    procedure CalculatePostedAssemblyCostExp(PostedAssemblyHeader: Record "Posted Assembly Header"; var ExpCost: array[5] of Decimal)
    begin
        GLSetup.Get;
    ...
    local procedure TestRtngVersionIsCertified(RtngVersionCode: Code[20]; RtngHeader: Record "Routing Header"): Boolean
    begin
    ...
    procedure DynAllItemBrowse()
    var
        lItem: Record Item;
    begin
        lItem.SetFilter("Costing Method", '<=%1', lItem."costing method"::Average);
        lItem.SetFilter("Replenishment System", '<>%1', lItem."replenishment system"::Assembly);

        if lItem.FindFirst() then
            repeat
                ;
                CalcItem(lItem."No.", false);
            until lItem.Next = 0;


        Message('done %1', lItem.Count);
    end;
}
